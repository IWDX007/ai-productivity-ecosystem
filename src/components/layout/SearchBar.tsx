...
import { ALL_SEARCH_TOOLS, type SearchToolItem } from "@/lib/searchToolsData";
...
export function SearchBar() {
  ...
  useEffect(() => {
    ...
    setFilteredTools(filtered.slice(0, 15));
  }, [query]);
  ...
  // Close only when open + click outside button AND panel
  useEffect(() => {
    if (!isOpen) return;
    ...
    // delay attach so opening click does not instantly close
    const t = setTimeout(() => {
    ...
    <>
      ...
      {isOpen ? (
        <div className="fixed inset-0 z-[9999]">
          ...
          <div className="relative z-10 flex items-start justify-center pt-16 sm:pt-24 px-4">
            <div
              ref={panelRef}
              className="w-full max-w-2xl bg-theme-card border border-theme rounded-2xl shadow-2xl overflow-hidden"
            >
              <div className="relative border-b border-theme">
                ...
                <input
                  ...
                  className="w-full pl-12 pr-16 py-4 bg-transparent text-theme-primary text-base outline-none placeholder:text-theme-muted"
                />
                <button
                  type="button"
                  onClick={closeSearch}
                  className="absolute right-3 top-1/2 -translate-y-1/2 px-2 py-1 text-xs bg-theme-secondary text-theme-primary rounded border border-theme"
                >
                  ESC
                </button>
              </div>
                ...
                {query.trim() ? (
                  filteredTools.length > 0 ? (
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="flex-1">
                              <p className="text-sm font-medium text-theme-primary group-hover:text-crimson-500">
                                {tool.name}
                              </p>
                              <p className="text-xs text-theme-muted">{tool.category}</p>
                            </div>
                            <ArrowRight className="w-4 h-4 text-theme-muted opacity-0 group-hover:opacity-100 transition-all" />
                          </Link>
                        );
                      })}
                    </div>
                    ...
                    <div className="p-8 text-center">
                      <Search className="w-10 h-10 text-crimson-500 mx-auto mb-3" />
                      <p className="text-theme-primary font-medium mb-1">No tools found</p>
                      <p className="text-sm text-theme-secondary">Try pdf, json, uuid, word counter...</p>
                    </div>
                  )
                ) : (
                  <>
