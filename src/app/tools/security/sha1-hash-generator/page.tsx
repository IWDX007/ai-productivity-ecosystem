...
export async function generateMetadata(): Promise<Metadata> {
  const tool = await getToolData("security", "sha1-hash-generator");
  ...
  return (
    <>
      ...
      <ToolSchema
        name={tool.name}
        description={tool.metaDescription || tool.description || ""}
        url={`https://ai-productivity-ecosystem-azure.vercel.app/tools/security/sha1-hash-generator`}
        category="security"
    ...
    />
      <SEOSections toolSlug="sha1-hash-generator" toolName={tool.name} category="security" />
    </>
  );
}