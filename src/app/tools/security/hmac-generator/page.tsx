...
export async function generateMetadata(): Promise<Metadata> {
  const tool = await getToolData("security", "hmac-generator");
  ...
  return (
    <>
      ...
      <ToolSchema
        name={tool.name}
        description={tool.metaDescription || tool.description || ""}
        url={`https://ai-productivity-ecosystem-azure.vercel.app/tools/security/hmac-generator`}
        category="security"
    ...
    />
      <SEOSections toolSlug="hmac-generator" toolName={tool.name} category="security" />
    </>
  );
}