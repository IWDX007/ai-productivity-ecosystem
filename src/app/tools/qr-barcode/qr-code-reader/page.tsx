...
export default function QrCodeReaderPage() {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    img.onload = async () => {
      ...
      if (ctx) {
        ...
        if (!found) setError("No QR code found in image")
      }
    }
  ...
  return (
    <div className="min-h-screen bg-theme-primary">
      <ToolPageMeta title="QR Code Reader" description="Scan QR codes using your camera or upload an image. Instantly decode URLs, text, WiFi credentials and more." keywords="qr code reader, free online tool, qr-code-reader, qr-barcode tools, ai productivity" />
      <div className="container mx-auto px-4 py-8 max-w-7xl">
        ...
        <div className="text-center mb-12 mt-6">
          ...
          <p className="text-theme-secondary max-w-2xl mx-auto">
            Scan QR codes using your camera or upload an image. Instantly decode 
            URLs, text, WiFi credentials and more.
          </p>
        </div>
        ...
        <div className="glass-card rounded-2xl p-6 md:p-8 mb-8 max-w-3xl mx-auto">
          ...
          <div className="grid grid-cols-2 gap-3 mb-6">
            ...
            <button onClick={scanning ? stopCamera : startCamera} className="py-3 bg-orange-500 hover:bg-orange-600 text-white font-semibold rounded-lg transition flex items-center justify-center gap-2">
              {scanning ? <><X className="w-5 h-5" /> Stop Camera</> : <><Camera className="w-5 h-5" /> Use Camera</>}
            </button>
          </div>
          ...
          {scanning && (
            <div className="mb-6">
              ...
              <div className="text-center text-sm text-theme-muted mt-2">Point camera at QR code</div>
            </div>
          )}
          ...
          {result && (
            <div className="p-4 rounded-lg bg-orange-500/10 border border-orange-500/30">
              <div className="flex items-center justify-between mb-2">
                <span className="text-sm text-orange-400 font-semibold">Decoded Content:</span>
                <button onClick={handleCopy} className="text-theme-muted hover:text-orange-400 transition">
                  {copied ? <Check className="w-5 h-5 text-green-400" /> : <Copy className="w-5 h-5" />}
                </button>
              </div>
              <div className="text-orange-300 font-mono text-sm break-all">{result}</div>
              {result.startsWith("http") && (
                <a href={result} target="_blank" rel="noopener noreferrer" className="inline-block mt-3 text-sm text-orange-400 hover:underline">
                  Open Link →
                </a>
              )}
            </div>
        ...
        <SEOSections toolSlug="qr-code-reader" toolName="QR Code Reader" category="QR & Barcode" />
      </div>
    </div>
  )
}