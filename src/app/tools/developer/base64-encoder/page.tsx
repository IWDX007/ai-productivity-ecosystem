...
export async function generateMetadata(): Promise<Metadata> {
  const tool = await getToolData("developer", "base64-encoder");
  ...
  return (
    <>
      ...
      <ToolSchema
        name={tool.name}
        description={tool.metaDescription || tool.description || ""}
        url={`https://ai-productivity-ecosystem-azure.vercel.app/tools/developer/base64-encoder`}
        category="developer"
    ...
    />
      <SEOSections toolSlug="base64-encoder" toolName={tool.name} category="developer" />
    </>
  );
}