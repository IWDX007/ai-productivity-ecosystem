...
const UNITS: Record<string, { name: string; symbol: string; toGrams: number }> = {
  ...
  kilogram:   { name: "Kilogram",   symbol: "kg",   toGrams: 1000 },
  metricTon:  { name: "Metric Ton", symbol: "t",    toGrams: 1000000 },
  ounce:      { name: "Ounce",      symbol: "oz",   toGrams: 28.3495 },
  pound:      { name: "Pound",      symbol: "lb",   toGrams: 453.592 },
  stone:      { name: "Stone",      symbol: "st",   toGrams: 6350.29 },
  usTon:      { name: "US Ton",     symbol: "ton",  toGrams: 907185 },
  ukTon:      { name: "UK Ton",     symbol: "lt",   toGrams: 1016047 },
  carat:      { name: "Carat",      symbol: "ct",   toGrams: 0.2 },
}
...
export default function WeightConverterPage() {
  ...
  const conversions = useMemo(() => {
    const fromFactor = UNITS[fromUnit].toGrams
    const valueInGrams = numericValue * fromFactor
    return Object.entries(UNITS).map(([key, unit]) => ({
      key, name: unit.name, symbol: unit.symbol,
      value: valueInGrams / unit.toGrams,
    }))
  ...
  const handleReset = () => { setValue("1"); setFromUnit("kilogram") }

  return (
    <div className="min-h-screen bg-theme-primary">
      <ToolPageMeta title="Weight Converter" description="Convert between kilograms, grams, pounds, ounces, tons, stones and more. Free online weight converter with no login required." keywords="weight converter, free online tool, weight-converter, converters tools, ai productivity" />
      <div className="container mx-auto px-4 py-8 max-w-7xl">
        ...
        <div className="text-center mb-12 mt-6">
          ...
          <h1 className="text-4xl md:text-5xl font-bold mb-4 text-theme-primary">
            Weight <span className="gradient-text">Converter</span>
          </h1>
          <p className="text-theme-secondary max-w-2xl mx-auto">
            Convert between kilograms, grams, pounds, ounces, tons, stones and more.
            Free online weight converter with no login required.
          </p>
        </div>
        ...
        <SEOSections toolSlug="weight-converter" toolName="Weight Converter" category="Converter" />
      </div>
    </div>
  )
}