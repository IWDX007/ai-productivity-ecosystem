...
const UNITS: Record<string, { name: string; symbol: string; toBase: number; type: string }> = {
  ...
  kbDec:      { name: "KB (decimal)",  symbol: "KB",  toBase: 8000,        type: "decimal" },
  kbBin:      { name: "KiB (binary)",  symbol: "KiB", toBase: 8192,        type: "binary" },
  mbDec:      { name: "MB (decimal)",  symbol: "MB",  toBase: 8e6,         type: "decimal" },
  mbBin:      { name: "MiB (binary)",  symbol: "MiB", toBase: 8388608,     type: "binary" },
  gbDec:      { name: "GB (decimal)",  symbol: "GB",  toBase: 8e9,         type: "decimal" },
  gbBin:      { name: "GiB (binary)",  symbol: "GiB", toBase: 8589934592,  type: "binary" },
  tbDec:      { name: "TB (decimal)",  symbol: "TB",  toBase: 8e12,        type: "decimal" },
  tbBin:      { name: "TiB (binary)",  symbol: "TiB", toBase: 8.796e12,    type: "binary" },
  pbDec:      { name: "PB (decimal)",  symbol: "PB",  toBase: 8e15,        type: "decimal" },
  pbBin:      { name: "PiB (binary)",  symbol: "PiB", toBase: 9.007e15,    type: "binary" },
}
...
export default function DigitalStorageConverterPage() {
  ...
  const conversions = useMemo(() => {
    ...
    const valueInBits = numericValue * fromFactor
    return Object.entries(UNITS).map(([key, unit]) => ({
      key, name: unit.name, symbol: unit.symbol, type: unit.type,
      value: valueInBits / unit.toBase,
    }))
  ...
  const handleReset = () => { setValue("1"); setFromUnit("gbDec") }

  return (
    <div className="min-h-screen bg-theme-primary">
      <ToolPageMeta title="Digital Storage Converter" description="Convert between decimal (KB, MB, GB) and binary (KiB, MiB, GiB) units. Understand the difference between manufacturer and OS storage sizes." keywords="digital storage converter, free online tool, digital-storage-converter, converters tools, ai productivity" />
      <div className="container mx-auto px-4 py-8 max-w-7xl">
        ...
        <div className="text-center mb-12 mt-6">
          ...
          <h1 className="text-4xl md:text-5xl font-bold mb-4 text-theme-primary">
            Digital Storage <span className="gradient-text">Converter</span>
          </h1>
          <p className="text-theme-secondary max-w-2xl mx-auto">
            Convert between decimal (KB, MB, GB) and binary (KiB, MiB, GiB) units.
            Understand the difference between manufacturer and OS storage sizes.
          </p>
        </div>
        ...
        <div className="glass-card rounded-2xl p-6 md:p-8 mb-8 max-w-4xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {conversions.map(({ key, name, symbol, type, value: cv }) => (
              <div key={key} className={`p-4 rounded-lg border transition ${key === fromUnit ? "bg-purple-500/10 border-purple-500/30" : type === "binary" ? "bg-blue-500/5 border-blue-500/20" : "bg-theme-secondary border-theme"} hover:border-purple-500/30`}>
        ...
        <SEOSections toolSlug="digital-storage-converter" toolName="Digital Storage Converter" category="Converter" />
      </div>
    </div>
  )
}