...
const UNITS: Record<string, { name: string; symbol: string; toMps: number }> = {
  ...
  mph:     { name: "Miles/hour",    symbol: "mph",  toMps: 0.44704 },
  fps:     { name: "Feet/sec",      symbol: "ft/s", toMps: 0.3048 },
  knot:    { name: "Knot",          symbol: "kn",   toMps: 0.514444 },
  mach:    { name: "Mach",          symbol: "M",    toMps: 343 },
  cmps:    { name: "cm/sec",        symbol: "cm/s", toMps: 0.01 },
  kmps:    { name: "Km/sec",        symbol: "km/s", toMps: 1000 },
  mps_min: { name: "Meters/min",    symbol: "m/min",toMps: 0.0166667 },
  mps_hr:  { name: "Meters/hour",   symbol: "m/h",  toMps: 0.000277778 },
}
...
export default function SpeedConverterPage() {
  ...
  const conversions = useMemo(() => {
    const fromFactor = UNITS[fromUnit].toMps
    const valueInMps = numericValue * fromFactor
    return Object.entries(UNITS).map(([key, unit]) => ({
      key, name: unit.name, symbol: unit.symbol,
      value: valueInMps / unit.toMps,
    }))
  ...
  const handleReset = () => { setValue("1"); setFromUnit("kph") }

  return (
    <div className="min-h-screen bg-theme-primary">
      <ToolPageMeta title="Speed Converter" description="Convert between mph, kph, m/s, knots, Mach and more speed units. Instant results for driving, aviation and sports." keywords="speed converter, free online tool, speed-converter, converters tools, ai productivity" />
      <div className="container mx-auto px-4 py-8 max-w-7xl">
        ...
        <div className="text-center mb-12 mt-6">
          ...
          <h1 className="text-4xl md:text-5xl font-bold mb-4 text-theme-primary">
            Speed <span className="gradient-text">Converter</span>
          </h1>
          <p className="text-theme-secondary max-w-2xl mx-auto">
            Convert between mph, kph, m/s, knots, Mach and more speed units.
            Instant results for driving, aviation and sports.
          </p>
        </div>
        ...
        <SEOSections toolSlug="speed-converter" toolName="Speed Converter" category="Converter" />
      </div>
    </div>
  )
}