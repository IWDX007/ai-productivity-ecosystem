...
const UNITS: Record<string, { name: string; symbol: string; toJoules: number }> = {
  ...
  megajoule:   { name: "Megajoule",     symbol: "MJ",    toJoules: 1e6 },
  calorie:     { name: "Calorie",       symbol: "cal",   toJoules: 4.184 },
  kilocalorie: { name: "Kilocalorie",   symbol: "kcal",  toJoules: 4184 },
  wattHour:    { name: "Watt-hour",     symbol: "Wh",    toJoules: 3600 },
  kWh:         { name: "Kilowatt-hour", symbol: "kWh",   toJoules: 3.6e6 },
  mWh:         { name: "Megawatt-hour", symbol: "MWh",   toJoules: 3.6e9 },
  btu:         { name: "BTU",           symbol: "BTU",   toJoules: 1055.06 },
  therm:       { name: "Therm",         symbol: "thm",   toJoules: 1.055e8 },
  eV:          { name: "Electronvolt",  symbol: "eV",    toJoules: 1.602e-19 },
  ftLb:        { name: "Foot-pound",    symbol: "ft-lb", toJoules: 1.35582 },
}
...
export default function EnergyConverterPage() {
  ...
  const conversions = useMemo(() => {
    const fromFactor = UNITS[fromUnit].toJoules
    const valueInJoules = numericValue * fromFactor
    return Object.entries(UNITS).map(([key, unit]) => ({
      key, name: unit.name, symbol: unit.symbol,
      value: valueInJoules / unit.toJoules,
    }))
  ...
  const handleReset = () => { setValue("1"); setFromUnit("kWh") }

  return (
    <div className="min-h-screen bg-theme-primary">
      <ToolPageMeta title="Energy Converter" description="Convert between Joules, calories, kilowatt-hours, BTU and more energy units. Perfect for physics, nutrition and electricity bills." keywords="energy converter, free online tool, energy-converter, converters tools, ai productivity" />
      <div className="container mx-auto px-4 py-8 max-w-7xl">
        ...
        <div className="text-center mb-12 mt-6">
          ...
          <h1 className="text-4xl md:text-5xl font-bold mb-4 text-theme-primary">
            Energy <span className="gradient-text">Converter</span>
          </h1>
          <p className="text-theme-secondary max-w-2xl mx-auto">
            Convert between Joules, calories, kilowatt-hours, BTU and more energy units.
            Perfect for physics, nutrition and electricity bills.
          </p>
        </div>
        ...
        <SEOSections toolSlug="energy-converter" toolName="Energy Converter" category="Converter" />
      </div>
    </div>
  )
}