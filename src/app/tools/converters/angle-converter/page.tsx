...
const UNITS: Record<string, { name: string; symbol: string; toRadians: number }> = {
  degree:      { name: "Degree",       symbol: "Ã‚Â°",    toRadians: Math.PI / 180 },
  radian:      { name: "Radian",       symbol: "rad",  toRadians: 1 },
  gradian:     { name: "Gradian",      symbol: "grad", toRadians: Math.PI / 200 },
  turn:        { name: "Turn",         symbol: "tr",   toRadians: 2 * Math.PI },
  arcminute:   { name: "Arcminute",    symbol: "'",    toRadians: Math.PI / 10800 },
  arcsecond:   { name: "Arcsecond",    symbol: '"',    toRadians: Math.PI / 648000 },
  milliradian: { name: "Milliradian",  symbol: "mrad", toRadians: 0.001 },
  quadrant:    { name: "Quadrant",     symbol: "quad", toRadians: Math.PI / 2 },
  sextant:     { name: "Sextant",      symbol: "sext", toRadians: Math.PI / 3 },
  revolution:  { name: "Revolution",   symbol: "rev",  toRadians: 2 * Math.PI },
}
...
export default function AngleConverterPage() {
  ...
  const conversions = useMemo(() => {
    const fromFactor = UNITS[fromUnit].toRadians
    const valueInRadians = numericValue * fromFactor
    return Object.entries(UNITS).map(([key, unit]) => ({
      key, name: unit.name, symbol: unit.symbol,
      value: valueInRadians / unit.toRadians,
    }))
  ...
  const handleReset = () => { setValue("90"); setFromUnit("degree") }

  return (
    <div className="min-h-screen bg-theme-primary">
      <ToolPageMeta title="Angle Converter" description="Convert between degrees, radians, gradians, turns, arcminutes and more. Perfect for math, engineering and navigation." keywords="angle converter, free online tool, angle-converter, converters tools, ai productivity" />
      <div className="container mx-auto px-4 py-8 max-w-7xl">
        ...
        <div className="text-center mb-12 mt-6">
          ...
          <h1 className="text-4xl md:text-5xl font-bold mb-4 text-theme-primary">
            Angle <span className="gradient-text">Converter</span>
          </h1>
          <p className="text-theme-secondary max-w-2xl mx-auto">
            Convert between degrees, radians, gradians, turns, arcminutes and more.
            Perfect for math, engineering and navigation.
          </p>
        </div>
        ...
        <SEOSections toolSlug="angle-converter" toolName="Angle Converter" category="Converter" />
      </div>
    </div>
  )
}