...
import TextMergerClient from "./TextMergerClient";
import SEOSections from "@/components/tools/SEOSections";
import { ToolSchema } from "@/components/seo/SchemaMarkup";

export async function generateMetadata(): Promise<Metadata> {
  const tool = await getToolData("text", "text-merger");
  ...
  return (
    <>
      ...
      <ToolSchema
        name={tool.name}
        description={tool.metaDescription || tool.description || ""}
        url={`https://ai-productivity-ecosystem-azure.vercel.app/tools/text/text-merger`}
        category="text"
    ...
    />
      <SEOSections toolSlug="text-merger" toolName={tool.name} category="text" />
    </>
  );
}