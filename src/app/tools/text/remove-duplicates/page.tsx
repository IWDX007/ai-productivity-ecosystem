...
export async function generateMetadata(): Promise<Metadata> {
  const tool = await getToolData("text", "remove-duplicates");
  ...
  return (
    <>
      ...
      <ToolSchema
        name={tool.name}
        description={tool.metaDescription || tool.description || ""}
        url={`https://ai-productivity-ecosystem-azure.vercel.app/tools/text/remove-duplicates`}
        category="text"
    ...
    />
      <SEOSections toolSlug="remove-duplicates" toolName={tool.name} category="text" />
    </>
  );
}