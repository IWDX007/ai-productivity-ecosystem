...
export async function generateMetadata(): Promise<Metadata> {
  const tool = await getToolData("text", "text-diff");
  ...
  return (
    <>
      ...
      <ToolSchema
        name={tool.name}
        description={tool.metaDescription || tool.description || ""}
        url={`https://ai-productivity-ecosystem-azure.vercel.app/tools/text/text-diff`}
        category="text"
    ...
    />
      <SEOSections toolSlug="text-diff" toolName={tool.name} category="text" />
    </>
  );
}