...
export default function Page() {
  ...
  return (
    <>
                ...
                {diff.map((d, i) => (
                  <div key={i} className="grid grid-cols-2 divide-x divide-theme text-xs font-mono">
                    <div className={`p-2 ${bg(d.status === "added" ? "" : d.status)}`}>
                      ...
                      <span className="text-theme-primary">{d.left || <span className="opacity-30">—</span>}</span>
                    </div>
                    <div className={`p-2 ${bg(d.status === "removed" ? "" : d.status)}`}>
                      <span className="text-theme-muted mr-2">{d.line}</span>
                      <span className="text-theme-primary">{d.right || <span className="opacity-30">—</span>}</span>
                    </div>
                  </div>
                ))}
              </div>
      ...
      <SEOSections toolSlug="text-diff" toolName="Text Diff Checker" />
    </>
  )
}