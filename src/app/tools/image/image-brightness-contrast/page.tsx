...
import ImageBrightnessContrastClient from "./ImageBrightnessContrastClient";
import SEOSections from "@/components/tools/SEOSections";
import { ToolSchema } from "@/components/seo/SchemaMarkup";

export async function generateMetadata(): Promise<Metadata> {
  const tool = await getToolData("image", "image-brightness-contrast");
  ...
  return (
    <>
      ...
      <ToolSchema
        name={tool.name}
        description={tool.metaDescription || tool.description || ""}
        url={`https://ai-productivity-ecosystem-azure.vercel.app/tools/image/image-brightness-contrast`}
        category="image"
        faqs={tool.seoFaqs || undefined}
        steps={tool.seoSteps || undefined}
        rating={tool.seoRating || undefined}
      />
      <ImageBrightnessContrastClient 
      name={tool.name}
      description={tool.description || ""}
    />
      <SEOSections toolSlug="image-brightness-contrast" toolName={tool.name} category="image" />
    </>
  );
}