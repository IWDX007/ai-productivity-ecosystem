  ...
  return (
    <div className="min-h-screen bg-theme-primary">
      ...
      <div className="container mx-auto px-4 py-8 max-w-7xl">
            ...
            <>
              ...
              {totalCompressed > 0 && (
                <div className="mb-4 p-3 rounded-lg bg-green-500/10 border border-green-500/30 text-green-400 text-sm">
                  Total: {formatSize(totalOriginal)} â†’ {formatSize(totalCompressed)} ({totalSavings}% saved)
                </div>
              )}
                  ...
                  <div key={idx} className="flex items-center gap-3 p-3 rounded-lg bg-theme-secondary border border-theme">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs text-theme-muted">
                        ...
                        {f.status === "done" && (
                          <> â†’ {formatSize(f.compressed)} <span className="text-green-400">({Math.round(((f.original - f.compressed) / f.original) * 100)}% saved)</span></>
                        )}
                      </div>
                    </div>
                    {f.status === "processing" && <Loader2 className="w-5 h-5 text-blue-400 animate-spin" />}
                    {f.status === "done" && <div className="text-green-400 text-xs">âœ“</div>}
        ...
        <SEOSections toolSlug="bulk-image-compressor" toolName="Bulk Image Compressor" category="Image" />
      </div>
    </div>
  )
}