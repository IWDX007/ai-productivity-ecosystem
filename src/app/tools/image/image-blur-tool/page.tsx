...
import ImageBlurToolClient from "./ImageBlurToolClient";
import SEOSections from "@/components/tools/SEOSections";
import { ToolSchema } from "@/components/seo/SchemaMarkup";

export async function generateMetadata(): Promise<Metadata> {
  const tool = await getToolData("image", "image-blur-tool");
  ...
  return (
    <>
      ...
      <ToolSchema
        name={tool.name}
        description={tool.metaDescription || tool.description || ""}
        url={`https://ai-productivity-ecosystem-azure.vercel.app/tools/image/image-blur-tool`}
        category="image"
    ...
    />
      <SEOSections toolSlug="image-blur-tool" toolName={tool.name} category="image" />
    </>
  );
}