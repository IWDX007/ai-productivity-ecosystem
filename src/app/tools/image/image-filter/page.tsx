...
export async function generateMetadata(): Promise<Metadata> {
  const tool = await getToolData("image", "image-filter");
  ...
  return (
    <>
      ...
      <ToolSchema
        name={tool.name}
        description={tool.metaDescription || tool.description || ""}
        url={`https://ai-productivity-ecosystem-azure.vercel.app/tools/image/image-filter`}
        category="image"
    ...
    />
      <SEOSections toolSlug="image-filter" toolName={tool.name} category="image" />
    </>
  );
}